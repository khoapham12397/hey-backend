...
public class WsServer {
    ...
    public Future<Void> createWsServer(Vertx vertx) {
        ...
        vertx.createHttpServer().websocketHandler(new Handler<ServerWebSocket>() {
            ...
            public void handle(final ServerWebSocket ws) {
                final String id = ws.textHandlerID(); // nhin  tao da : // dung y
                // no vao 1 cai la no co cai moi luon dung vay do :
                // sau do the nap ?
                
                String query = ws.query();
               
                System.out.println("AAA: COMMUNICATE BETWEEN SOCKET CLIENT AND SOCKET SERVER in thread : "+ Thread.currentThread().getId()+" and at websocket: "+ ws.hashCode() );
    ...
    // khi ma no di vao thi ro rang la vay do dung vay do ???
    // dieu nay hop ly do ma just do it???
    // sau do the nao ??
    // ro rang dieu nay the nao la tot nhat ???
    // dung vay do:
    // di theo 2 huong duoc ko ??
